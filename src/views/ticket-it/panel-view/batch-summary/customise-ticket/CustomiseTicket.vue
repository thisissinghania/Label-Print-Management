      ...
      >
        <v-row v-if="singleTicketEdit?.data">
          ...
          <v-col cols="12" sm="6">
            <v-row>
              <v-col
                v-for="(value, fieldName) in singleTicketEdit?.data?.fields"
                :key="fieldName"
                class="custom-field"
                cols="12"
              >
                ...
                <v-col class="field-type pa-0" v-if="singleTicketEdit?.data?.types[fieldName] == 1">
                  <Field
                    :name="fieldName"
                    type="text"
                ...
                <v-col class="field-type pa-0" v-if="singleTicketEdit?.data?.types[fieldName] == 2">
                  <Field
                    v-slot="{ field }"
                    :name="fieldName"
                    v-if="value && !allListItemsData[fieldName]"
                ...
                <v-col class="field-type pa-0" v-if="singleTicketEdit?.data?.types[fieldName] == 3">
                  <Field
                    :name="fieldName"
                    type="date"
                    v-if="value && !allListItemsData[fieldName]"
                ...
                <v-col class="field-type pa-0" v-if="singleTicketEdit?.data?.types[fieldName] == 4">
                  <Field
                    :name="fieldName"
                    as="select"
                    v-if="value && !allListItemsData[fieldName]"
                  ...
                  >
                    ...
                    <option
                      :value="titles"
                      :key="titles"
                      v-for="(val, titles) in singleTicketEdit?.data?.lists[fieldName]"
                    >
                      {{ titles }}
                    </option>
                  </Field>
                ...
                <v-col class="field-type pa-0" v-if="singleTicketEdit?.data?.types[fieldName] == 5">
                ...
                <v-col class="field-type pa-0" v-if="singleTicketEdit?.data?.types[fieldName] == 6">
                  <Field
                    :name="fieldName"
                    type="text"
                    v-if="value && !allListItemsData[fieldName]"
                ...
                <v-col class="field-type pa-0" v-if="singleTicketEdit?.data?.types[fieldName] == 7">
                  <div class="end-search-icon searchable-field">
                    <Field
                      :name="fieldName"
                      v-slot="{ field }"
                ...
                <v-col
                  ...
                  v-if="singleTicketEdit?.data?.types[fieldName] == 8"
                >
                  <Field
                    :name="fieldName"
                    as="select"
              ...
              <v-col cols="12" class="mt-1" v-if="singleTicketEdit?.data?.fields">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const submit = async (values) => {
      ...
      const formModelFields = singleTicketEdit.value?.data?.fields
      const dataWithDefaults = setDefaultValues(values, formModelFields)
      const data = {
        updatingFields: dataWithDefaults,
        batchId: batch,
        templateName: singleTicketEdit?.value?.data?.tName,
        ticketId: ticketIdVal?.value
      }
      await store.dispatch(Actions.SINGLE_TICKET_UPDATE, data)
    }
    ...
    const previewApiFunction = async (allPayloads) => {
      const batch = new URLSearchParams(window.location.search).get('batchId')
      const formModelFields = singleTicketEdit.value?.data?.fields
      const payloadWithDefaults = setDefaultValues(allPayloads, formModelFields)
      const ticketParams = {
        editKeys: payloadWithDefaults,
        templateName: singleTicketEdit?.value?.data?.tName,
        ticketId: ticketIdVal?.value,
        batchId: batch
      }
      await store.dispatch(Actions.PROMO_PREVIEW_JSON, ticketParams)
    }
    ...
    watch(singleTicketEdit, (newVal) => {
      if (newVal) {
        loader.value = false
        previewApiFunction(singleTicketEdit?.value?.data?.fields)
      }
    })
    watch(singleTicketUpdate, (newVal) => {
      if (newVal) {
        loader.value = false
        emit('closeModal')
      }
